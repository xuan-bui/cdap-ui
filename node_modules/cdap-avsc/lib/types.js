  ...
  function ConstructorFunction(oArgs) {
    ...
    var innerFunction = ({
      [constructorName]: function (...iArgs) {
        ...
        self._fields.forEach((field, i) => {
          getDefault = field.getDefault;
          name = field._name;
          if (getDefault() === undefined) {
            this[`${name}`] = iArgs[i];
          } else {
            if (iArgs[i] === undefined) {
              this[`${name}`] = oArgs[i]();
            } else {
              this[`${name}`] = iArgs[i];
            }
    ...
    var innerFunction = ({
      [constructorName]: function(v, f, h, p) {
        ...
        if (h) {
          ...
          self._fields.forEach((field, i) => {
            p[j] = field._name;
            b &= field.getDefault() === undefined ?
              outerArgs[i]._check(v[field._name], f, h, p)
              :
          ...
          return self._fields.reduce((prevVal, currentVal, i) => {
            return prevVal && (
              currentVal.getDefault() === undefined ?
                outerArgs[i]._check(v[currentVal._name])
                :
  ...
  function ConstructorFunction(outerArgs) {
    ...
    var innerFunction = ({
      [constructorName]: function(t) {
        return new outerArgs[0](...outerArgs.slice(1).map(aa => aa['_read'](t)));
      }
    })[constructorName];
    return innerFunction;
  }
...
RecordType.prototype._createSkipper = function () {
  ...
  return ConstructorFunction(values); + 1
};
...
RecordType.prototype._createWriter = function () {
  ...
  this._fields.forEach((field, i) => {
    values.push(field._type);
    if (field.getDefault() !== undefined) {
      dValues[i] = field._type.toBuffer(field.getDefault()).toString('binary');
    }
  });
  var self = this;
  function ConstructorFunction(vv, dv) {
    ...
    var innerFunction = ({
      [constructorName]: function(t, v) {
        self._fields.forEach((field, i) => {
          if (field.getDefault() === undefined) {
            vv[i]._write(t, v[field._name]);
          } else {
            if (v[field._name] === undefined) {
          ...
          if (i === lazyIndex) {
            if (!b) {
              if (resolvers[name] === undefined) {
                ...
                let index = innerArgs.indexOf(resolvers[name].name);
                innerArgs[index] = resolvers[name].resolver._read(t);
              }
            }
          } else {
            if (resolvers[name] === undefined) {
